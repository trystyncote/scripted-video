...
def test_parser_default():
    ...
    expected = ["aAaA;", "bBbB;", "cCcC; // Commented out?", "dDdD", "eEeE;", "fFfF;", "gGgG; /*", "Commented out?",
                "*/", "hHhH", "i;"]
    for actual in script_parser("example_script_file_1.txt"):
...
def test_parser_inline_comments():
    ...
    expected = ["aAaA;", "bBbB;", "cCcC;", "dDdD", "eEeE;", "fFfF;", "gGgG; /*", "Commented out?",
                "*/", "hHhH", "i;"]
    for actual in script_parser("example_script_file_1.txt", inline_comment_character="//"):
        assert actual == expected[idx]
        idx += 1
...
def test_parser_block_comments():
    ...
    for actual in script_parser("example_script_file_1.txt", block_comment_characters=("/*", "*/")):
        assert actual == expected[idx]
        idx += 1
...
def test_parser_end_line():
    ...
    for actual in script_parser("example_script_file_1.txt", end_line_character=";"):
        assert actual == expected[idx]
        idx += 1
